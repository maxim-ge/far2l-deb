...
Чтобы на безыксовом сервере заработал gvfs-плагин, не забудьте сделать `sudo apt install gvfs-fuse`, запускать `sudo killall gvfsd && dbus-launch far2l`.
...
To use gvfs-plugin on X11-less server, don't forget to do `sudo apt install gvfs-fuse`, then run `sudo killall gvfsd && dbus-launch far2l`.
